function pool2conv_mem(layer_idx)
    global config mem;
    ...
    mem.pool2conv{layer_idx+1} = {};    
    RM_final = RM_final(:);
    mem.pool2conv{layer_idx+1}{1} = RM_final(1:length(RM_final)-config.kernel_size(conv_layer_idx, 1)*config.kernel_size(conv_layer_idx, 2)*invalid_kernel_num);
    mem.pool2conv{layer_idx+1}{1} = config.NEW_MEM(mem.pool2conv{layer_idx+1}{1});
    
    mem.pool2conv{layer_idx+1}{2} = [];
    for m = 1:config.feature_map_sizes{layer_idx}(3)*config.batch_size-1
        mem.pool2conv{layer_idx+1}{2} = [mem.pool2conv{layer_idx+1}{2} m*valid_kernel_num+(m-1)*invalid_kernel_num+1:m*valid_kernel_num+(m-1)*invalid_kernel_num+invalid_kernel_num];
    end
    mem.pool2conv{layer_idx+1}{2} = config.NEW_MEM(mem.pool2conv{layer_idx+1}{2});
end
